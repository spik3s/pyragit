...
func TestAppRendersFleet(t *testing.T) {
	...
	if !strings.Contains(view, "fFetch ") || !strings.Contains(view, "NNew wt ") || !strings.Contains(view, "qQuit ") {
	...
	if !strings.Contains(view, "↵Diff ") || !strings.Contains(view, "yCopy path ") || strings.Contains(view, "fFetch ") {
		t.Errorf("files key bar wrong:\n%s", view)
	}
	m = drive(t, m, key("l"))
	if v := ansi.Strip(m.View().Content); !strings.Contains(v, "j/kScroll ") || !strings.Contains(v, "↵Back ") {
